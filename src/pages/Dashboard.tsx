  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        ...
        <SummaryCard icon={CheckCircle2} label="완료" value={`${completed.length}건`} color="bg-green-50 text-green-600" />
        <SummaryCard icon={Clock} label="대기/확정" value={`${confirmed.length + pending.length}건`} color="bg-blue-50 text-blue-600" />
        <SummaryCard icon={TrendingUp} label="오늘 매출" value={`₩${todayRevenue.toLocaleString()}`} color="bg-amber-50 text-amber-600" />
      </div>
          ...
          {upcomingBookings.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {upcomingBookings.map((b) => {
                ...
                return (
                  <div key={b.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                    <div className="w-1 h-10 rounded-full" style={{ backgroundColor: staff?.profileColor }} />
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={dailySales}>
                ...
                />
                <Area type="monotone" dataKey="total" stroke="#6172F3" strokeWidth={2} fill="url(#colorSales)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
              ...
              {staffList.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[11px] font-bold" style={{ backgroundColor: s.profileColor }}>
                      {s.name[0]}
                    </div>
                    <span className="text-[13px] text-gray-700 flex-1">{s.name}</span>
                    <span className="text-[13px] font-bold text-gray-800">{count}건</span>
                  </div>
                );
              })}
            </div>
